...
class TranslationCache {
  private cache = new Map<
    string,
    { data: NestedTranslationObject; timestamp: number; size: number }
  >();
  private totalSize = 0;
  ...
  async get(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (
          result &&
          Date.now() - result.timestamp < DEFAULT_CACHE_CONFIG.maxAge
        ) {
          resolve(result.data);
        } else {
          resolve(null);
...
class LanguageAnalytics {
  private usage = new Map<
    SupportedLanguage,
    { count: number; lastUsed: number; avgDuration: number }
  >();

  recordUsage(language: SupportedLanguage, duration?: number): void {
    const current = this.usage.get(language) || {
      count: 0,
      lastUsed: 0,
      avgDuration: 0,
    };
    ...
    if (duration) {
      current.avgDuration =
        (current.avgDuration * (current.count - 1) + duration) / current.count;
    }
  ...
  private calculatePredictionScore(data: {
    count: number;
    lastUsed: number;
    avgDuration: number;
  }): number {
    ...
    try {
      ...
      if (stored) {
        ...
        const entries = Object.entries(data)
          ...
          .map(
            ([lang, stats]) =>
              [lang as SupportedLanguage, stats] as [
                SupportedLanguage,
                { count: number; lastUsed: number; avgDuration: number },
              ],
          );
  ...
  try {
    ...
    if (translations) {
      ...
      await persistentCache
        .set(language, translations, namespace)
        .catch(console.warn);
    }
...
export async function smartPreload(
  ...
  const predictedLanguages =
    languageAnalytics.predictNextLanguages(currentLanguage);
