...
export const LANGUAGE_KEYBOARD_SHORTCUTS: Record<SupportedLanguage, KeyboardShortcuts> = {
  en: {
...
export function createLanguageChangeAnnouncement(
  ...
  const previousDirection = previousLanguage ? (isRTLLanguage(previousLanguage) ? "rtl" : "ltr") : "ltr";
...
export function announceToScreenReader(
  ...
  let announcer = document.getElementById("nara-screen-reader-announcer") as HTMLElement;
 ...
 */
export function getKeyboardShortcuts(language: SupportedLanguage): KeyboardShortcuts {
...
export function registerKeyboardShortcuts(
  ...
  const handleKeydown = (event: KeyboardEvent) => {
    ...
    if (handlers.languageSwitcher && shortcuts.languageSwitcher.includes(combo)) {
    ...
    if (handlers.navigation) {
      for (const [action, shortcutKeys] of Object.entries(shortcuts.navigation)) {
    ...
    if (handlers.textFormatting) {
      for (const [action, shortcutKeys] of Object.entries(shortcuts.textFormatting)) {
  ...
  listeners.push({ element: document, event: "keydown", handler: handleKeydown });
