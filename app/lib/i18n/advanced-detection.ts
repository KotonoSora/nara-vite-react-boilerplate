...
import { SUPPORTED_LANGUAGES, DEFAULT_LANGUAGE, isSupportedLanguage } from "./config";
...
export const LANGUAGE_REGIONS: Record<SupportedLanguage, string[]> = {
  ...
  es: ["ES", "MX", "AR", "CO", "VE", "PE", "CL", "EC", "GT", "CU", "BO", "DO", "HN", "PY", "SV", "NI", "CR", "PA", "UY"],
  fr: ["FR", "CA", "BE", "CH", "LU", "MC", "SN", "CI", "BF", "ML", "NE", "TG", "BJ", "CF", "TD"],
  zh: ["CN", "TW", "HK", "MO", "SG"],
  hi: ["IN", "NP"],
  ar: ["SA", "AE", "QA", "KW", "BH", "OM", "JO", "LB", "SY", "IQ", "YE", "EG", "LY", "TN", "DZ", "MA", "SD"],
  vi: ["VN"],
  ja: ["JP"],
  th: ["TH"],
} as const;
...
export function detectLanguageAdvanced(options: LanguageDetectionOptions = {}): LanguageDetectionResult {
  ...
  if (acceptLanguageHeader) {
    ...
    if (browserResult.language !== DEFAULT_LANGUAGE) {
      ...
      method = "browser";
      finalFallbackChain = [detectedLanguage, ...browserResult.fallbackChain, ...finalFallbackChain];
    }
  }
  ...
  if (region && confidence < 0.8) {
    ...
    if (geoResult.language !== DEFAULT_LANGUAGE && geoResult.confidence > confidence) {
      detectedLanguage = geoResult.language;
      confidence = geoResult.confidence;
      method = "geographic";
      finalFallbackChain = [detectedLanguage, ...geoResult.fallbackChain, ...finalFallbackChain];
    }
  }
  ...
  if (timezone && confidence < 0.6) {
    ...
    if (timezoneResult.language !== DEFAULT_LANGUAGE && timezoneResult.confidence > confidence) {
      detectedLanguage = timezoneResult.language;
      confidence = timezoneResult.confidence;
      method = "geographic";
      finalFallbackChain = [detectedLanguage, ...timezoneResult.fallbackChain, ...finalFallbackChain];
    }
  }
 ...
 */
function detectFromAcceptLanguage(acceptLanguage: string): LanguageDetectionResult {
  if (!acceptLanguage) {
    return {
  ...
  const languages = acceptLanguage
    ...
    .map((lang) => {
      ...
      return {
        ...
        mapped: BROWSER_LANGUAGE_MAP[normalizedCode] || 
  ...
  const fallbackChain = languages.slice(1).map(l => l.mapped!).filter(Boolean);
...
function detectFromRegion(region: string): LanguageDetectionResult {
  ...
  for (const [language, regions] of Object.entries(LANGUAGE_REGIONS)) {
    if (regions.includes(upperRegion)) {
      ...
      const confidence = calculateRegionalConfidence(language as SupportedLanguage, upperRegion);
      return {
        language: language as SupportedLanguage,
        confidence,
        method: "geographic",
        fallbackChain: getRegionalFallbacks(language as SupportedLanguage, upperRegion),
      };
    }
  }
...
function detectFromTimezone(timezone: string): LanguageDetectionResult {
  const timezoneMap: Record<string, { language: SupportedLanguage; confidence: number }> = {
 ...
 * Calculate confidence based on language prevalence in region
 */
function calculateRegionalConfidence(language: SupportedLanguage, region: string): number {
 ...
 */
function getRegionalFallbacks(language: SupportedLanguage, region: string): SupportedLanguage[] {
 ...
 */
function getTimezoneBasedFallbacks(language: SupportedLanguage): SupportedLanguage[] {
 ...
 */
export function saveUserLanguagePreferences(preferences: UserLanguagePreferences): void {
  try {
...
export function suggestLanguageForUser(
  ...
  // Add fallback chain
  suggestions.push(...detection.fallbackChain.filter(lang => lang !== currentLanguage));
