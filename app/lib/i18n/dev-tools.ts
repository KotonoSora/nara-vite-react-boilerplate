...
export interface ValidationError {
  type: "missing" | "invalid_format" | "broken_interpolation";
  key: string;
  ...
  validateAll(): ValidationResult {
    ...
    for (const key of unusedKeys) {
      warnings.push({
        ...
        suggestion:
          "Consider removing unused translations to reduce bundle size",
      });
    }
  ...
  validateKey(
    ...
    if (translation.length > 500) {
      warnings.push({
        ...
        suggestion:
          "Consider breaking down long translations into smaller parts",
      });
    }

    // Encoding checks removed to simplify validation in strict environments

    return { valid: true, errors, warnings };
  }

  private extractAllKeys(
    obj: NestedTranslationObject | Record<string, unknown>,
    prefix = "",
  ): string[] {
    const keys: string[] = [];
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === "string") {
        ...
        keys.push(
          ...this.extractAllKeys(value as Record<string, unknown>, fullKey),
        );
      }
    }
...
export class AutoTranslationSuggester {
  ...
  suggestTranslation(
    ...
    return this.generateBasicSuggestion(
      sourceText,
      sourceLanguage,
      targetLanguage,
    );
  ...
  translate(
    ...
    if (translation === key) {
      ...
      if (englishTranslation !== key && language !== "en") {
        const suggestion = this.suggester.suggestTranslation(
          key,
          "en",
          language,
          englishTranslation,
