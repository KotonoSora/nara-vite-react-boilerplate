...
export const CULTURAL_COLORS: Record<SupportedLanguage, {
  primary: string;
  success: string;
  warning: string;
  danger: string;
...
export function formatPhoneNumber(
  ...
  if (match) {
    let formatted = format.format.replace(/\$(\d+)/g, (_, group) => match[parseInt(group)]);
...
export function formatName(
  name: { first?: string; middle?: string; last?: string; father?: string; grandfather?: string; family?: string; maternal?: string },
  language: SupportedLanguage,
  ...
  for (const component of format.order) {
    ...
    if (value) {
      ...
      if (formal && component === "middle" && (language === "en" || language === "es" || language === "fr")) {
...
export function isColorCulturallyAppropriate(
  ...
  if (context === "mourning") {
    ...
    if (colors.auspicious.includes(color.toUpperCase())) {
      return { appropriate: false, reason: "This color is associated with celebrations" };
    }
  }
  ...
  if (context === "celebration") {
    ...
    if (colors.inauspicious.includes(color.toUpperCase())) {
      return { appropriate: false, reason: "This color is considered inauspicious" };
    }
  }
...
export function getFormattingExamples(language: SupportedLanguage) {
  return {
    ...
    phone: formatPhoneNumber("1234567890", language, { includeCountryCode: true }),
    name: formatName(
