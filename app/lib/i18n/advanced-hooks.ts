...
import type { Address } from "./cultural-formatting";
import type { PerformanceMetrics } from "./performance";
import type { TranslationContext, TranslationKey } from "./types";
...
export function useLanguageDetection() {
  ...
  const [preferences, setPreferences] =
    useState<UserLanguagePreferences | null>(null);
  ...
  const saveLanguagePreference = useCallback(
    (
      ...
      method: UserLanguagePreferences["detectionMethod"],
    ) => {
...
export function useI18nAccessibility() {
  ...
  const announceLanguageChange = useCallback(
    (newLanguage: SupportedLanguage, immediate = false) => {
      const announcement = createLanguageChangeAnnouncement(
        newLanguage,
        previousLanguage,
      );
      announceToScreenReader(
        announcement,
        immediate ? "assertive" : "polite",
        newLanguage,
      );
...
export function useI18nPerformance() {
  ...
  const preloadSpecificLanguages = useCallback(
    async (languages: SupportedLanguage[], namespaces?: string[]) => {
      await preloadLanguages(languages, namespaces);
    },
    [],
  );

  const recordLanguageUsage = useCallback(
    (duration?: number) => {
...
export function useUserLanguagePreferences() {
  ...
  const savePreferences = useCallback(
    (newPreferences: Partial<UserLanguagePreferences>) => {
...
export function useAdvancedTranslation() {
  ...
  const [translationManager, setTranslationManager] =
    useState<AdvancedTranslationManager | null>(null);

  // Initialize advanced translation manager
  useEffect(() => {
    if (!advancedTranslationManager) {
      advancedTranslationManager = new AdvancedTranslationManager(language, [
        "en",
      ]);
  ...
  const validateTranslations = useCallback(
    (language: string) => {
      return (
        translationManager?.validateTranslations(language) || {
          missing: [],
          incomplete: [],
          warnings: [],
        }
