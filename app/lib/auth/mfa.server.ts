...
export async function generateMFASecret(
  ...
  const backupCodes = Array.from({ length: 8 }, () => generateRandomString(8));
...
export async function disableMFA(db: Database, userId: number) {
  await db.delete(mfaSecret).where(eq(mfaSecret.userId, userId));
}
...
export async function getUserMFAStatus(db: Database, userId: number) {
  ...
  return {
    ...
    hasBackupCodes: userMFA
      ? JSON.parse(userMFA.backupCodes || "[]").length > 0
      : false,
