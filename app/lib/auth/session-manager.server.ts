...
import { generateSessionId, getSessionExpiry } from "~/lib/auth/config";
...
export async function invalidateSession(
  ...
  const result = await db.delete(session).where(eq(session.id, sessionId));
...
export async function invalidateAllUserSessions(
  ...
  // Log global logout
  if (metadata) {
    await logSecurityEvent(db, userId, "global_logout", "session", {
      ...metadata,
      sessionCount,
...
export async function cleanupExpiredSessions(
  ...
  const deletedCount =
    (result as any)?.changes || (result as any)?.meta?.changes || 0;
