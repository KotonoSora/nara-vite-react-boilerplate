...
function windowStartFrom(now: Date, windowMs: number): Date {
...
function resetTimeFrom(
  ...
  const base = windowStart ?? now;
  return base.getTime() + windowMs;
}

async function fetchCurrentWindowRecord(
...
function buildRateLimitHeaders(
  ...
  if (!result.allowed) headers["Retry-After"] = secondsUntil(result.resetTime);
  return headers;
}
...
export async function checkRateLimit(
  ...
  const now = new Date();
  const windowStart = windowStartFrom(now, config.windowMs);

  // Get or create current window record
  const existingRecord = await fetchCurrentWindowRecord(
    db,
    identifier,
    endpoint,
    windowStart,
  ...
  let resetTime = resetTimeFrom(
    existingRecord?.windowStart ?? null,
    config.windowMs,
    now,
  );
  ...
  if (existingRecord) {
    ...
    currentAttempts = 1;
    resetTime = resetTimeFrom(now, config.windowMs, now);
  }
...
export async function cleanupRateLimitLogs(
  ...
  await db.delete(rateLimitLog).where(lt(rateLimitLog.createdAt, cutoff));
}
  ...
  return async function rateLimitMiddleware(
    ...
    if (!result.allowed) {
      throw new Response("Too Many Requests", {
        status: 429,
        headers: buildRateLimitHeaders({ ...result, remaining: 0 }, config),
      });
    }
...
export function addRateLimitHeaders(
  ...
  const rlHeaders = buildRateLimitHeaders(result, config);
...
function secondsUntil(epochMs: number): string {
  return String(Math.max(0, Math.ceil((epochMs - Date.now()) / 1000)));
}
...
export async function getRateLimitStatus(
  ...
  const now = new Date();
  const windowStart = windowStartFrom(now, config.windowMs);
  const record = await fetchCurrentWindowRecord(
    db,
    identifier,
    endpoint,
    windowStart,
  ...
  if (!record) {
    return {
      ...
      resetTime: resetTimeFrom(null, config.windowMs, now),
      totalAttempts: 0,
    };
  }
  ...
  const resetTime = resetTimeFrom(record.windowStart, config.windowMs, now);
