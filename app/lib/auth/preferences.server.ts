...
export async function getUserPreferences(
  ...
  return {
    ...
    fallbackLanguages: prefs.fallbackLanguages
      ? JSON.parse(prefs.fallbackLanguages)
      : ["en"],
    ...
    notifications: prefs.notifications
      ? JSON.parse(prefs.notifications)
      : {
          email: true,
          security: true,
          marketing: false,
...
export async function updateLanguagePreference(
  ...
  await updateUserPreferences(db, userId, {
    ...
    fallbackLanguages: fallbacks || [
      language,
      ...currentPrefs.fallbackLanguages.filter((l) => l !== language),
    ],
  });
}
...
export function detectPreferredLanguage(
  ...
  if (geoLocation?.country) {
    ...
    const countryLanguage =
      countryLanguageMap[geoLocation.country.toUpperCase()];
    if (countryLanguage) {
...
export function formatDateWithPreferences(
  ...
  try {
    ...
    if (options.relative) {
      ...
      if (diffHours < 24) {
        const rtf = new Intl.RelativeTimeFormat(preferences.preferredLanguage, {
          numeric: "auto",
        });
