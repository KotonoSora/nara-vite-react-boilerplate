...
export async function initializePermissions(
  ...
  const uniquePermissions = allPermissions.filter(
    (perm, index, self) =>
      self.findIndex((p) => p.name === perm.name) === index,
  );
      ...
      try {
        ...
        if (existingPermission) {
          await db
            ...
            .onConflictDoNothing({
              target: [rolePermission.role, rolePermission.permissionId],
            });
        }
      } catch (error) {
        console.warn(
          `Failed to assign permission ${perm.name} to role ${role}`,
        );
      }
    }
  }
 ...
 */
export function createPermissionChecker(db: DrizzleD1Database<typeof schema>) {
  return {
    ...
    async requireResourceAction(
      ...
      if (!canPerform) {
        throw new Response(`Forbidden: Cannot ${action} ${resource}`, {
          status: 403,
        });
      }
    },
