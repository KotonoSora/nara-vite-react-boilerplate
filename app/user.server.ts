...
import type { OAuthProvider, OAuthUserInfo } from "~/lib/auth/oauth.server";
...
export async function authenticateUser(
  ...
  try {
    ...
    if (foundUser && metadata) {
      await logLoginAttempt(db, foundUser.id, success, {
        ...metadata,
        provider: null,
      });
    }
...
export async function getRecentLoginAttempts(
  ...
  return await db
    .select()
    .from(loginLog)
    .where(and(eq(loginLog.userId, userId), gt(loginLog.createdAt, since)))
...
export async function unlinkOAuthAccount(
  ...
  const result = await db
    .delete(oauthAccount)
    .where(
      and(eq(oauthAccount.userId, userId), eq(oauthAccount.provider, provider)),
    );
