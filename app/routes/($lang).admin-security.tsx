...
import { getSecurityAuditLogs } from "~/lib/auth/device-tracking.server";
...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  const authResult = await requireCriticalSecurity(request, db, ["admin.manage"]);

  // Get additional security information
  const auditLogs = await getSecurityAuditLogs(db, authResult.userId, 50);
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  const { logSecurityEvent } = await import("~/lib/auth/device-tracking.server");
  
  await logSecurityEvent(db, {
    userId: authResult.userId,
    action: "admin_security_action",
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-red-700">
                    <p>
                      This area requires critical-level security authentication including:
                      session/API authentication, basic auth, and admin permissions.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <dl className="space-y-2">
                  <div>
                    ...
                    <dd className="text-sm text-gray-900">{loaderData.user?.name} ({loaderData.user?.email})</dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-600">Auth Flow:</dt>
                  ...
                  <div>
                    ...
                    <dd className="text-sm text-red-600 font-semibold uppercase">{loaderData.securityLevel}</dd>
                  </div>
                  <div>
