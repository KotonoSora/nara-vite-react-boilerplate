...
export function useLazyExpansion({
  ...
  useEffect(() => {
    ...
    if (topGap < bufferWeeks && mode === "date") {
      const needUp = bufferWeeks - topGap; // how many extra weeks to add above
    ...
    if (
      (bottomGap < bufferWeeks && mode === "date") ||
      (bottomGap < bufferWeeks &&
        mode === "sequence" &&
        maxWeekIndex < DEFAULT_MAX_WEEK_MODE_SEQUENCE)
    ) {
      const needDown = bufferWeeks - bottomGap; // how many extra weeks to add below
