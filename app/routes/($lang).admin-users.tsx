...
import { getUserSession, requireUserId } from "~/auth.server";
...
export async function loader({ request, context, params }: Route.LoaderArgs) {
  ...
  await requireAdminRole(db, userId);

  // Get users created by this admin
  const managedUsers = await getUsersCreatedBy(db, userId);

  // Log security access
  await logSecurityAccess(db, userId, request, "/admin-users", "high", {
    ...
    switch (actionType) {
      ...
      case "update": {
        const targetUserId = parseInt(
          formData.get("userId")?.toString() || "0",
        );
        const name = formData.get("name")?.toString();
        const email = formData.get("email")?.toString();
        ...
        const result = await updateUserByAdmin(db, userId, targetUserId, {
          name,
          email,
        });
      ...
      case "delete": {
        ...
        const result = await deleteUserByAdmin(db, userId, targetUserId);
    ...
    <div className="container mx-auto p-6">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">
          {t("admin.userManagement.userManagement") ||
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">
              {t("admin.userManagement.createNewUser") || "Create New User"}
            </h2>
            <button
          ...
          {showCreateForm && (
            <Form method="post" className="space-y-4">
              ...
              >
                {t("admin.userManagement.createUser") || "Create User"}
              </button>
            </Form>
          )}
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-xl font-semibold">
              {t("admin.userManagement.managedUsers") || "Managed Users"} (
              {managedUsers.length})
            </h2>
          </div>
          ...
          {managedUsers.length === 0 ? (
            <div className="p-6 text-center text-gray-500">
              {t("admin.userManagement.noUsers") || "No users created yet"}
            </div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {t("admin.userManagement.created") || "Created"}
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
