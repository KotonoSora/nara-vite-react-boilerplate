...
export const rateLimit = (options: RateLimitOptions) =>
  createMiddleware<{ Bindings: Env }>(async (c, next) => {
    ...
    try {
      ...
      // Increment counter
      await c.env.RATE_LIMIT_KV.put(cacheKey, (count + 1).toString(), {
...
app.get("/posts/:id", async (c) => {
  ...
  // Cache for 5 minutes
  await c.env.CACHE_KV.put(`post:${id}`, JSON.stringify(post), {
    expirationTtl: 300,
  });
...
export const createAuthMiddleware = (requiredRoles?: string[]) =>
  createMiddleware<{ Bindings: Env }>(async (c, next) => {
    ...
    try {
      ...
      const isBlacklisted = await c.env.TOKEN_BLACKLIST_KV.get(
        `token:${token}`,
      );
      if (isBlacklisted) {
          ...
          return c.json(
            {
              ...
              retryAfter: Math.ceil(
                (validRequests[0] + options.windowMs - now) / 1000,
              ),
            },
            429,
          );
...
export const servicesMiddleware = createMiddleware<{ Bindings: Env }>(
  async (c, next) => {
  ...
  private async handleJobFailure(job: Job, error: any) {
    if (job.retryCount < job.maxRetries) {
      ...
      job.scheduledAt = new Date(
        Date.now() + Math.pow(2, job.retryCount) * 1000,
      ); // Exponential backoff
  ...
  createMiddleware(async (c, next) => {
    try {
      ...
      if (!result.success) {
        ...
        return c.json(
          formatError("Validation failed", "VALIDATION_ERROR", {
...
export const globalErrorHandler = createMiddleware(async (c, next) => {
  try {
    ...
    if (error instanceof ApiError) {
      return c.json(
        formatError(error.message, error.code, error.context),
        error.statusCode,
      );
    }
    ...
    if (error instanceof z.ZodError) {
      return c.json(
        formatError("Validation failed", "VALIDATION_ERROR", error.flatten()),
        400,
      );
    }
    ...
    if (error.message.includes("UNIQUE constraint failed")) {
      ...
      return c.json(
        formatError("Resource already exists", "UNIQUE_VIOLATION", { field }),
        409,
      );
    }
...
export const requestSizeLimit = (maxSize: number) =>
  createMiddleware(async (c, next) => {
    ...
    if (contentLength && parseInt(contentLength) > maxSize) {
      return c.json(
        formatError("Request entity too large", "REQUEST_TOO_LARGE", {
          maxSize,
        }),
        413,
      );
