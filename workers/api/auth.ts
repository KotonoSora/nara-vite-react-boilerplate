...
import { extractBearerToken, getDb } from "~/workers/api/helpers";
...
/**
 * API Tokens management (session-based for logged-in users)
 * GET/POST/DELETE /api/auth/tokens
 */
auth.get("/tokens", async (c): Promise<Response> => {
  const db = getDb(c.env.DB, schema);
  const userId = await getUserId(c.req.raw);
  if (!userId) return c.json({ error: "Unauthorized" }, 401);

  // Rate limit general access
  const rateLimiters = createRateLimiters(db);
  try {
    await rateLimiters.apiGeneral(c.req.raw, "/api/auth/tokens", userId);
  } catch (rateLimitResponse) {
    return rateLimitResponse as Response;
  }

  const tokens = await getUserAPITokens(db, userId);
  return c.json({ tokens, availableScopes: Object.values(API_SCOPES) });
});

auth.post("/tokens", async (c): Promise<Response> => {
  const db = getDb(c.env.DB, schema);
  const userId = await getUserId(c.req.raw);
  if (!userId) return c.json({ error: "Unauthorized" }, 401);

  // Strict rate limit for creation
  const rateLimiters = createRateLimiters(db);
  try {
    await rateLimiters.apiStrict(c.req.raw, "/api/auth/tokens", userId);
  } catch (rateLimitResponse) {
    return rateLimitResponse as Response;
  }
  ...
  if (contentType.includes("application/json")) {
    ...
    if (Array.isArray(scopesInput)) scopes = scopesInput;
    ...
    if (body?.expiresInDays)
      expiresInDays = Number(body.expiresInDays) || undefined;
  } else {
    const form = await c.req.formData();
    name = String(form.get("name") ?? "");
    const scopesString = String(form.get("scopes") ?? "");
    if (scopesString)
      scopes = scopesString
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
    const expRaw = form.get("expiresInDays");
    if (expRaw) expiresInDays = Number(expRaw) || undefined;
  }

  if (!name || name.length < 3) {
    return c.json({ error: "Token name must be at least 3 characters" }, 400);
  }

  // Validate scopes
  const validScopes = Object.values(API_SCOPES);
  const invalidScopes = scopes.filter((s) => !validScopes.includes(s as any));
...
auth.delete("/tokens", async (c): Promise<Response> => {
  const db = getDb(c.env.DB, schema);
  const userId = await getUserId(c.req.raw);
  if (!userId) return c.json({ error: "Unauthorized" }, 401);

  // Strict rate limit for revoke
  const rateLimiters = createRateLimiters(db);
  try {
    await rateLimiters.apiStrict(c.req.raw, "/api/auth/tokens", userId);
  } catch (rateLimitResponse) {
  ...
  if (contentType.includes("application/json")) {
    ...
    const form = await c.req.formData();
    tokenId = Number(form.get("tokenId"));
  }
